...
app.post('/api/reference', cors(), async (req, res) => {
  try {
    ...
    console.error('[API /api/reference Error]', err);
    return res.status(500).json({ success: false, error: err.message || '서버 오류가 발생했습니다.' });
  }
});
...
Return ONLY valid JSON matching this schema.`;
  ...
  try {
    ...
    console.error('Gemini Search Query generation error in local server:', e);
  }
  ...
  const industryImagePool = getFilteredPoolServer(industry, customRequirements);
  ...
  if (references.length === 0) {
    ...
    references = industryImagePool.map((seed, idx) => ({
      ...
      searchSourceUrl: getArchSourceSearchUrl(sourceList[idx % sourceList.length], industry, style, seed.zone),
